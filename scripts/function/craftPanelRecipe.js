...
export class CraftPanelRecipe extends HandlebarsApplication {
    ...
    async _prepareContext(options) {
        ...
        debug("CraftPanelRecipe _prepareContext : elementItems_items", elementItems_items);
        const elementItems = elementItems_items.map((item, i) => {
        ...
        debug("CraftPanelRecipe _prepareContext : elementItems", elementItems);
        const recipesJE = this.journalEntry.pages.filter(p => p.flags[MODULE_ID]?.type === "recipe");
    ...
    async configure() {
        const fb = new Portal.FormBuilder()
            ...
            .select({ name: `flags.${MODULE_ID}.mergeByName`, label: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.merge-by-name`), options: { "default": game.i18n.localize(`${MODULE_ID}.default`), "yes": game.i18n.localize(`${MODULE_ID}.yes`), "no": game.i18n.localize(`${MODULE_ID}.no`) } })
