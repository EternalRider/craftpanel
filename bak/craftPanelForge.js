...
export class CraftPanelForge extends HandlebarsApplication {
    constructor(journalEntry, mode = "edit", options = {}) {
        ...
        this.slotItems = {};
        this.slotMaterials = [];
        this.keepMaterials = game.user?.getFlag(MODULE_ID, "keepMaterials") ?? false;
        ...
        debug("CraftPanelForge constructor : this journalEntry mode options craftPanels", this, journalEntry, mode, options, craftPanels);
    }
    ...
    async _prepareContext(options) {
        ...
        debug("CraftPanelForge _prepareContext: slotsJE", slotsJE);
        ...
        debug("CraftPanelForge _prepareContext: this.slots", this.slots);
        ...
        debug("CraftPanelForge _prepareContext: results", results);
        ...
        debug("CraftPanelForge _prepareContext: modifiers", modifiers);
    ...
    _onRender(context, options) {
        ...
        debug("CraftPanelForge _prepareContext: context", context);
        // 恢复滚动条位置
        ...
        const keepMaterialsInput = html.querySelector("input[name='keep-materials']");
        if (keepMaterialsInput) {
            keepMaterialsInput.checked = this.keepMaterials;
            keepMaterialsInput.addEventListener("change", async (event) => {
        ...
        debug("CraftPanelForge _onRender: html", html);
    }
    ...
    async configure() {
        ...
        const fb = new FormBuilder()
            ...
            .select({ name: `flags.${MODULE_ID}.shape`, label: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.shape`), options: { "default": game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.default`), ...CraftPanelForge.SHAPE_STYLE } })
            ...
            .multiSelect({ name: `flags.${MODULE_ID}.requirements`, label: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.panel-requirements`), hint: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.panel-requirements-hint`), options: { ...CraftPanelForge.REQUIREMENTS_TYPE_OPTIONS } })
        ...
        debug("CraftPanelForge configure: data", data);
        if (!data) return;
    ...
    async _onDropSlot(event) {
        ...
        debug("CraftPanelForge _onDropSlot: data", data);
        if (data.type !== "Item") return;
        const index = parseInt(event.currentTarget.dataset.index);
        const item = await fromUuid(data.uuid);
        debug("CraftPanelForge _onDropSlot: index item", index, item);
        if (item) {
    ...
    async _onDropSlotPanel(event) {
        ...
        if ((data.type == "CraftSlot") && this.isEdit) {
            ...
            debug("CraftPanelForge _onDropSlotPanel: slot", slot);
            ...
            for (let i = 0; i < this.slots.length; i++) {
                if (!this.slots[i].isLocked && (this.slotItems[i] === null || this.slotItems[i] === undefined)) {
                    ...
                    const item = await fromUuid(data.uuid);
                    debug("CraftPanelForge _onDropSlotPanel : item", item);
                    await this.addIngredient(i, item);
                }
            }
        }
    ...
    async _onDropResultPanel(event) {
        ...
        debug("CraftPanelForge _onDropResultPanel: data", data);
        const type = data.type;
        const item = (data?.uuid ?? false) ? await fromUuid(data.uuid) : false;
        debug("CraftPanelForge _onDropResultPanel: type item", type, item);
    ...
    async _onDropModifierPanel(event) {
        ...
        debug("CraftPanelForge _onDropModifierPanel: data", data);
        if (data.type !== "Item" && data.type !== "ActiveEffect") return;
        const item = await fromUuid(data.uuid);
        debug("CraftPanelForge _onDropModifierPanel: item", item);
        let changes = [];
        ...
        debug("CraftPanelForge _onDropModifierPanel: changes", changes);
        await this.journalEntry.createEmbeddedDocuments("JournalEntryPage", [
            {
                name: item.name,
                src: item.img,
    ...
    async _onDropCostPanel(event) {
        ...
        debug("CraftPanelForge _onDropCostPanel: data", data);
        if (data.type !== "Item" && data.type !== "CraftElement") return;
        const item = (data?.uuid ?? false) ? await fromUuid(data.uuid) : false;
        debug("CraftPanelForge _onDropCostPanel: item", item);
        let element = data?.element;
        debug("CraftPanelForge _onDropCostPanel: element", element);
        if (type == "Item") {
    ...
    async _onClickResult(event) {
        ...
        if (this.isEdit || this.choosedResults.includes(result.uuid)) {
            ...
            if (this.isEdit) {
                ...
                fb.select({ name: `shape`, label: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.shape`), options: { "default": game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.default`), ...CraftPanelForge.SHAPE_STYLE } });
            }

            const data = await fb.render();
            debug("CraftPanelForge _onClickResult: data", data);
            if (!data) return;
    ...
    async _onClickSlot(event) {
        ...
        debug("CraftPanelForge _onClickSlot: index isEmpty", index, isEmpty);
        if (isEmpty) {
            return;
        } else {
            await this.removeIngredient(index);
    ...
    async _onClickCategory(event) {
        const category = event.currentTarget.dataset.category;
        const type = event.currentTarget.dataset.type;
        debug("CraftPanelForge _onClickCategory: category", category, "type", type);
        //如果是编辑模式，点击分类可以编辑分类
    ...
    async removeIngredient(index) {
        ...
        debug("CraftPanelForge removeIngredient: this.slotItems this.materials", this.slotItems, this.materials);
        //将材料整理成类似元素的格式
    ...
    async addIngredient(index, item, options = {}) {
        ...
        if (await this.checkAdd(index, item)) {
            ...
            debug("CraftPanelForge addIngredient: this.slotItems this.materials", this.slotItems, this.materials);
            //将材料整理成类似元素的格式
            this.refreshSlotMaterials();
            if (!skipRefresh) {
                await this.refreshElements();
    ...
    refreshSlotMaterials() {
        ...
        debug("CraftPanelForge refreshSlotMaterials: this.slotMaterials", this.slotMaterials);
    }
    /**
     * 刷新元素和结果
     */
    ...
    async refreshPanel() {
        ...
        debug("CraftPanelForge refreshPanel: this.material_categories", this.material_categories);
        //恢复之前选中的分类
        ...
        debug("CraftPanelForge refreshPanel: material_category", material_category);
        //刷新材料
        let materials_items = [];
        const requirements = {};
        ...
        debug("CraftPanelForge refreshPanel: requirements", requirements);
        ...
        debug("CraftPanelForge refreshPanel: materials_items", materials_items);
        this.materials = [];
        ...
        debug("CraftPanelForge refreshPanel: this.materials", this.materials);
        //仅在特定情况下刷新调整
    ...
    async refreshModifiers() {
        ...
        debug("CraftPanelForge refreshModifiers: this.modifier_categories", this.modifier_categories);
        //恢复之前选中的分类
        ...
        debug("CraftPanelForge refreshModifiers: modifier_category", modifier_category);
        //刷新调整
        ...
        debug("CraftPanelForge refreshModifiers: this.modifiers", this.modifiers);
        ...
        debug("CraftPanelForge refreshModifiers: this.choosedModifiers choosedCount this.modifierLimit", this.choosedModifiers, choosedCount, this.modifierLimit);
        ...
        debug("CraftPanelForge refreshModifiers: this.cost this.choosedModifiers", this.cost, this.choosedModifiers);
    }
    //刷新可用点数
    async refreshCost() {
        let cost = Number(this.baseCost ?? 0);
        let elementCost = 0;
        debug("CraftPanelForge refreshCost: cost elementCost", cost, elementCost);
        ...
        cost += elementCost;
        debug("CraftPanelForge refreshCost: cost", cost);
        ...
        this.cost.max = cost;
        debug("CraftPanelForge refreshCost: this.cost", this.cost);
    }
    //检查能否添加该物品到槽位中
    async checkAdd(index, item) {
        debug("CraftPanelForge checkAdd : index item this.slots[index] this.actor", index, item, this.slots[index], this.actor);
        if (!item) return false;
        if (this.slots[index].isLocked) return false;
        if (this.actor) {
            //检查数量
            let quantity = this.countQuantity(item);
            debug("CraftPanelForge checkAdd : quantity", quantity);
        ...
        debug("CraftPanelForge checkAdd : slotJE", slotJE);
        const config = slotJE.getFlag(MODULE_ID, "requirements") ?? [];
        debug("CraftPanelForge checkAdd : config", config);
        if (config.length > 0) {
            const requirements = {};
            config.forEach(key => {
                if (key == "script") {
            ...
            debug("CraftPanelForge checkAdd : requirements", requirements);
    ...
    checkSlot() {
        ...
        debug("CraftPanelForge checkSlot : slots", slots);
    ...
    countQuantity(item) {
        ...
        debug("CraftPanelForge countQuantity : item quantity", item, quantity);
        if (quantity === undefined) {
            return undefined;
        }
    ...
    async editSlot(slotJEUuid) {
        ...
        debug("CraftPanelForge editSlot: slotJE", slotJE);
        const showTypeOptions = {
        ...
        const fb = new FormBuilder()
            ...
            .multiSelect({ name: `flags.${MODULE_ID}.requirements`, label: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.slot-requirements`), hint: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.slot-requirements-hint`), options: { ...CraftPanelForge.REQUIREMENTS_TYPE_OPTIONS } })
    ...
    async editModifier(modifierJEUuid) {
        ...
        debug("CraftPanelForge editModifier: modifierJE", modifierJE);
    ...
    async chooseModifier(modifierJEUuid) {
        ...
        debug("CraftPanelForge chooseModifier: modifierJE modifier", modifierJE, modifier);
        if (modifier.auto) return;
        const cost = modifier.cost ?? modifierJE.getFlag(MODULE_ID, "cost") ?? 0;
        debug("CraftPanelForge chooseModifier: cost this.cost", cost, this.cost);
        if (this.choosedModifiers.includes(modifierJE.uuid)) {
            ...
            debug("CraftPanelForge chooseModifier: categories", categories);
    ...
    async addCategory(type) {
        debug("CraftPanelForge addCategory : type", type);
        const defaultData = {
        ...
        if (type == "material") {
            fb.tab({ id: "requirements", icon: "fas fa-list-check", label: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.configure-requirements-tab`) })
                .multiSelect({ name: `requirements`, label: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.category-requirements`), hint: game.i18n.localize(`${MODULE_ID}.${this.APP_ID}.category-requirements-hint`), options: { ...CraftPanelForge.REQUIREMENTS_TYPE_OPTIONS } })
        ...
        const data = await fb.render();
        debug("CraftPanelForge addCategory : data", data);
        if (!data) return;
        data.id = data.name;
    ...
    async editCategory(category, type) {
        ...
        debug("CraftPanelForge editCategory : index categories", index, categories);
        if (!categories[index]) {
        ...
        const data = await fb.render();
        debug("CraftPanelForge editCategory : data needDelete", data, needDelete);
        if (needDelete) {
    ...
    async craft() {
        ...
        debug("CraftPanelForge craft: preScript postScript", preScript, postScript);
        let materials = [];
        let results = [];
        this.canceled = false;
        const previousSlotItems = this.keepMaterials ? foundry.utils.deepClone(this.slotItems) : null;
        //整理所有的材料
        ...
        debug("CraftPanelForge craft: materials", materials);
        //执行预处理脚本
        ...
        debug("Hooks.call craftPanelForgePre", this, this.journalEntry, this.actor, this.modifiersJE, this.elements, materials, this.canceled);
        await Hooks.call("craftPanelForgePre", this, this.journalEntry, this.actor, this.modifiersJE, this.elements, materials, this.canceled);
        //获取当前选择的所有调整
        let selectedModifiers = this.modifiersJE.filter(m => this.choosedModifiers.includes(m.uuid));
        debug("CraftPanelForge craft: selectedModifiers", selectedModifiers);
        //获取合成结果
        for (let id of this.choosedResults) {
            const re = this.results.find(r => r.uuid == id);
            const item = await fromUuid(re.uuid);
            debug("CraftPanelForge craft: re item", re, item);
            if (item) {
                if (re.type == "Item") {
                    results.push({
                        item: item.toObject(),
                    ...
                    for (let j = 0; j < re.quantity; j++) {
                        ...
                        debug("CraftPanelForge craft: object", object);
                        for (const r of object.results) {
                            let uuid = r.documentCollection + "." + r.documentId;
                            debug("CraftPanelForge craft: r uuid", r, uuid);
                            ...
                            debug("CraftPanelForge craft: resultItem", resultItem);
                            if (!resultItem) {
                                // @ts-ignore
                            ...
                            debug("CraftPanelForge craft: result", result);
                            if (result) {
                                result.quantity++;
                            } else {
                                results.push({
        ...
        results = results.map(r => {
            debug("CraftPanelForge craft: r", r);
        ...
        debug("CraftPanelForge craft: results", results);
        //应用调整
        if (selectedModifiers.length > 0) {
            for (let modifier of selectedModifiers) {
                ...
                debug("CraftPanelForge craft: changes asAE", changes, asAE);
                if (asAE) {
                    if (asAE == "merge") {
                        for (let re of results) {
                            ...
                            if (ae) {
                                ...
                                ae = buildActiveEffect(re.name, re.img, changes, aeType);
                                re.item.effects ??= [];
        ...
        if (this.actor) {
            ...
            debug("CraftPanelForge craft: this.actor products", this.actor, products);
            materials.forEach(m => {
                let item = m.item;
            ...
            debug("CraftPanelForge craft: updates toDelete products", updates, toDelete, products);
            if (!this.canceled) {
        ...
        debug("Hooks.call craftPanelForgePost", this, this.journalEntry, this.actor, this.modifiersJE, this.elements, materials, results, this.canceled);
        await Hooks.call("craftPanelForgePost", this, this.journalEntry, this.actor, this.modifiersJE, this.elements, materials, results, this.canceled);
        this.elements = [];
        if (this.keepMaterials && !this.canceled && previousSlotItems) {
            this.slotItems = {};
            this.slotMaterials = [];
            for (const [slotIndex, slotData] of Object.entries(previousSlotItems)) {
